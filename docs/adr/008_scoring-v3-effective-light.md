...
**Date**: 2026-06-28
**Status**: Accepted（**v3 を UI の既定モデルに採用**。v2/v1 は選択可として温存）。**位置づけ: v3 は「気候除外スクリーン＋候補ドシエ生成器」であって、永住地の最終スカラー順位ではない**（cross-review breaker の指摘を反映、後述）。
...
- **UI 既定モデルを v3 に切り替え済み**（2026-06-29, ユーザー判断）。v2/v1 は選択可として温存（古い参照値 score_v2 / score_v3 も両方表示）。ただし「除外スクリーン＋ドシエ」の位置づけは不変＝上位序列は重み/アンカーに敏感（上述）。
